...
> 状态:**Phase 0 + Phase 1 已实施(0.2.110,de8588a);Phase 2 未做(暂缓,见 §3)**。
> 版本基准:DHS/develop @ 0.2.110。
...
### Phase 2 —— 可选实验(输出会变,再 10~100× 或质量提升)**【状态:未做,建议暂缓】**

> **这是什么 / 为什么还没做(通俗版)**:Phase 0/1 已把「每次 pack 调用」内部的判空/占位做成了**整数格子**方案——每个岛按 256×256 格子量化(`ceil((w+gap)/cell)`),所有重叠判断在整数格子上做。格子化有两个副作用:① 成本与格子数相关(每个岛要扫若干格);② **量化误差**——岛的浮点坐标被吸附到格子,「放不放得下」在边界处不精确(所以现在 k 要乘 0.999999 留余量)。Phase 2 的「去栅格」= 改用**浮点 AABB 矩形两两重叠**直接判断(岛数 n≤60 时 O(n²)≈3.6k 次比较),单次 pack 从 ~30 万格操作降到 ~µs 级,且无量化误差。
>
> **代价**:placement 结果与现在逐格量化版**不完全一致**(不是 bug,是两种布局);已用 0.2.110 导出过的贴图若重新导出会得到略不同的岛位置(影响复现性,需重新验收)。
>
> **建议**:暂缓。理由——① 真实工程已从 4.0s → ~456ms(导出 UX 无感);② 它破坏与 0.2.110 的逐位兼容;③ 优化到亚毫秒后 Worker 并行反而失去意义。仅当未来出现「千岛/任意形状岛」需求(那时转 xatlas 系)或用户明确要「布局再紧一点」时再评估。若要做,半天实验分支对比质量后决定。
