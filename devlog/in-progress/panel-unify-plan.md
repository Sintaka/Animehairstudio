...
>
> 状态：**第 0/1/2/4 步已落地并提交；第 3 步与第 5 步未做（用户拍板延后）。**
> 当前：`APP_VERSION = 0.1.5-Sintaka.0.2.180`，全量 **697 pass / 0 fail**（主脑亲跑）。
...
>
> **剩余**：第 3 步（depth 创作入口，含 `writeBackTierCurvesToLeaves` 的 key 列表，
> 两者必须同批）、第 5 步（缺陷⑤⑥ + 三个死槽，D3/D5 已拍板本轮不动）。
> 另：0.2.180 的中间层首笔播种**只对宽度键**走 bake 通路，depth 键维持回落，
> 补第 3 步时要一并处理。
