...
- [x] 导出 UV：Spread Islands to All Available Space 散布（0.2.86，分支 DHS/develop）：模仿 Houdini UV Layout SOP 的 `expandpadding`（Spread Islands to All Available Space）——`packFamilies` 在 Smart 择优后增 gap 二分（`SPREAD_KEEP=0.9`：散布后岛保留 ≥90% 线性尺寸；`SPREAD_GAP_MAX=0.1`：二分上界），找「该 gap 下 findMaxK 仍 ≥ best.k×0.9」的最大 gap，把打包后集中在右上角的空档均摊成岛间间隙，使 UV 岛散布填满 UDIM 1001 而非堆在左下角；返回对象新增 `gap`（spread 后实际用的 gap）；单岛（<2 family）或 `spread:false` 时保持原紧排（gap=PACK_GAP 不变）；tests/uv-pack.test.mjs 新增 spread 散布性 + 单岛不 spread 用例，既有间距断言改按 `result.gap` 校验
