...
- **不 merge，只移植**：本地已重构为 `modules/<domain>/` 分域 + `app.js` 编排层（38,369→17,461 行），与 main 的扁平结构正交；对 main 做简单 merge/rebase 只会引入大范围冲突，失去重构意义。main 更新一律先分析 diff，再**按功能移植**。
- **四类处置**：①可直接移植（新增模块/纯函数/自包含逻辑）；②需在本地架构重新接线（功能重叠）；③本地已实现（标 `deprecated` 并删本地实现）；④本地已用不同方案解决（跳过，仅记录决策）。
- **bug 修复先查本地**：每个 bug 修复先 grep 本地是否已修复或已用别法解决，再决定移植，避免重复劳动或覆盖本地方案。
- **记录**：每次评估/移植结论都追加到本文件；中文内容 UTF-8 无 BOM 写入。
