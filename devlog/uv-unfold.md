...
- **关键函数（0.2.110 更新）**：`packFamilies(families, {gap, fill})`（uv-pack.js，纯函数零依赖，返回 `{k, totalArea, fillUsed, packed:[{id,island,x,y,width,height}], gap}`）；**0.2.110 两段式重构**：新增 `preparePack`（valid/单位缩放/uvisland/boxUnit/orders/kMax）、`sampleMaxK`（采样索引段内最后一个放得下 k）、`refineMaxK`（24 二分细化）、`applyPackResult`（最终缩放+验证重试+平移+fit-to-tile），`packFamilies` = prepare → 每 seed sample+refine 择优 → apply（输出逐位不变）；`alpacaPackOccupancy` 内部换**行区间表**（每行二分判空 + 区间合并，替代栅格+积分图，逐格等价，旧实现注释保留）；**新增 Worker 池并行版 `packFamiliesAsync`**（uv-pack-async.js，8 seed×8 块 sample 任务 + 8 refine 任务，确定性合并与同步逐位一致，无 Worker 回退同步）；导出链路 `packUnfoldedUv`/`buildUnfoldedMeshes`/`buildHairObj`/`buildHairUsda` 已 async；当前打包器 `alpacaPackOccupancy` + `findMaxKAlpaca`；注释保留的旧实现 `maxRectsPack`/`findMaxK`/`alpacaPackTurbo`/`alpacaPack`；接线 project-files.js `packUnfoldedUv`；`tests/uv-pack.test.mjs` 纯 node 回归（含确定性压力回归 + fit-to-tile + 方形断言 + 0.2.110 冻结回归 fixtures/uv-pack-reference.json）+ `tests/uv-pack-async.test.mjs`（worker 池逐位一致/回退/dispose）。
