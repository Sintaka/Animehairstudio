...
- **Spread Islands to All Available Space（0.2.86）**：`packFamilies` 在 Smart 择优后增 gap 二分（`SPREAD_KEEP=0.9`、`SPREAD_GAP_MAX=0.1`），找「岛保留 ≥90% 线性尺寸」前提下的最大 gap，把打包后右上角空档均摊成岛间间隙（模仿 Houdini UV Layout 的 `expandpadding`）；单岛（<2 family）或 `spread:false` 跳过（紧排不变）；返回新增 `gap` 字段。默认开启（`spread=true`）。
- **关键函数**：`packFamilies(families, {gap, fill, spread})`（uv-pack.js，纯函数零依赖，返回 `{k, totalArea, fillUsed, packed:[{id,island,x,y,width,height}], gap}`）；接线 project-files.js `packUnfoldedUv`；`tests/uv-pack.test.mjs` 纯 node 回归（含确定性压力回归 + spread 散布性）。
