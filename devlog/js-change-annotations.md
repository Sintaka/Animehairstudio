...
- **Phase 2.15（Region 选区左右方向修正 + 选区记忆 + sweep 手柄选中 + 桥接接缝法线）**：版本 0.1.4-Sintaka.0.2.33。
  ...
  - **扫掠沿长度翻转修复（0.2.33，深挖保存 0041 扭动异常）**：用户保存 0041 后扫掠出现大扭动、而骨骼/gizmo 正常。定位：strandGeometryFrameAt 每帧朝 desiredZ（=退化 pointSurfaceNormals 投影 + authored twist）做 roll（每环 ±24° 上限），分支子级法线退化导致 roll 沿长度累积翻转——实测 up 从 row0 y=+0.064 翻到 row6 y=-0.012。种子（0.2.28）只稳了第一帧。修复：分支子级（lock.branchParentId）在传输时不再朝退化法线 roll，而是把 profile up 锚定到**传输的骨骼 up**（帧新增 untwistedZ，种子提供初始值），再直接施加 authored twist（z = ref·applyAxisAngle(tangent, twist)）；非分支走原逻辑。验证（保存的 0041，SL3）：row0/6/13/19/25 的 up y 分量始终为正（0.027/0.015/0.018/0.008），不再翻转，丝带跟随骨骼 + authored twist。
