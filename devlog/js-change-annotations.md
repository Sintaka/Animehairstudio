...
> - **仍未做的优化（已知未解项）**：同一行的截面对**所有 shell、所有 u 都相同**，现在每个顶点
>   都从 0 重新积分一遍。按行建**前缀和表**（采一次、各顶点查表插值）可再降一个数量级 ——
>   24×24 的 19.6ms 里绝大部分是这份重复积分。没做是因为它要改两个消费点的调用结构，
>   属独立改动；默认细分下 3.94ms 已跟手，极端细分才需要它。
...
> - **暴露出一个非本轮引入的不一致**：`tipPanelWidthAt` 与 `panelWidthAt` 在 `t=1`
>   （`TAPER_TO_ZERO` 收到 0）处取值不同 —— 网格侧截面退化成一个点、把手侧不为 0。旧模型
>   比较的是 delta（绝对差抵消），弧长参数化对截面形状是**非线性**的于是暴露出来（实测该行
>   差 0.013、其余行 <1e-5）。测试跳过该行并注明成因；修它要统一那两个函数在 t=1 的取值，
>   属独立改动。
