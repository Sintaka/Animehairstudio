...
- **Phase 2.15（Region 选区左右方向修正 + 选区记忆 + sweep 手柄选中 + 桥接接缝法线）**：版本 0.1.4-Sintaka.0.2.21。
  ...
  - **直接桥接不再末端多分一段（0.2.21）**：顶/底桥接带在选区上下过矮（topSegments/bottomSegments=1，直接桥接）时也会触发末端 0.3 处额外循环线（2.13 为侧收口补的），导致直接桥接被拆成 2 段。修复：顶/底统一改成只在填充桥接（分段>1，midCount>0）时才发末端循环线（emitTopMidRow/emitBottomMidRow(1 - 0.3/segments)），直接桥接段 midCount=0、单 quad 直连。验证（Sussurro_v1_0040，SL2 父/SL3 子）：直接配置（顶/底均 1 段）bridgeQuads=6（=顶2+底2+侧2，旧行为 10），当前配置（顶填充2段+底直接1段）bridgeQuads=10（顶仍保留端点环）。
