...
- **版本号规范（2026-08 起）**：主版本 `0.1.4` **与上游保持对齐**（不随本地改动递增）。`Sintaka` 后为**分段版本号 + dailybuild**：格式 `0.1.4-Sintaka.<分段版本号>.<dailybuild>`（如 `0.1.4-Sintaka.0.2.12.00001`）。分段版本号**对接 git graph / 开发阶段**（当前 `0.2.12`，功能迭代时更新）；dailybuild 为 5 位序号，**每次构建 +1，可累加到 5 位数**（00001..99999），分段版本号更新时清零。版本号写入 `modules/app-config.js` 的 `APP_VERSION`，显示在 Settings → Version。
