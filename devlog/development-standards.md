...
- **版本号规范（2026-08 起）**：个人开发版本号暂时以 `0.xxx.xxxxx` 构建（如 `0.001.00001`）：`0` 为主版本（正式发布前固定）；`xxx` 为功能版本号（3 位，功能更新迭代时 +1，即"大版本"先迭代）；`xxxxx` 为 daily build（5 位，每次构建 +1，可累加到 99999）；功能版本 +1 时 daily build 清零。版本号写入 `modules/app-config.js` 的 `APP_VERSION`，显示在 Settings → Version。
