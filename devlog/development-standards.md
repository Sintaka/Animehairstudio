...
| USD 骨骼/蒙皮导出（SkelBindingAPI + 单 SkelRoot + Hair_Root + 发丝名前缀关节 + row-vector 矩阵 + orient 派生 + Bones & Capture Mesh 菜单） | 启用 | 0.2.93–0.2.100：Mesh 施加 SkelBindingAPI（rel skel:skeleton + int[]/float[] jointIndices/jointWeights + elementSize）；单个 SkelRoot "Character"（含单个 Skeleton "Hair_Skel" + 所有蒙皮 mesh）；Hair_Root 空关节单根（x=0 正中线、发根平均）→ 单连通树；关节名 `${发丝名}_${i}` / `_split_${k}` / `_split_${k}_tip_${j}`（同名 lock lockPrefix 去重，joints token 相对 stage root 完整路径）；矩阵 row-vector（平移最后一行、旋转列向量转置、local=R·T、world=local·parent、局部偏移 p−parent.p）；面板 panelWeights 双影响 / 普通 bindBySweepRow 单影响；main 链 orient 从发丝切线派生（strandGeometryFrameAt → quatToMat3 3x3）；菜单 Bones & Capture Mesh 默认勾选 + Mesh 灰掉、Path Prefix（rootName 持久化 + 快速导出复用）、OBJ 直接导出、快速导出默认 usda、取消导出不下载。0.2.106：split 骨骼布局修复（`splitBoneLayout`/`bridgeRootParentName` 纯函数）——split 骨骼 parent 改 `main.${forkIdx}`（暴露段 fork 参数映射）+ 未创作时派生位置（panel 段尖 = tip 链末点、发丝管尖 = 曲线末端 + spread 侧向偏移），桥接子发片 `main.0` parent 到父发片对应骨骼点（不再挂 Hair_Root）。0.2.107：发尖暴露链导出（`splitChainLayout`）——split 骨骼 = 暴露链根 + 后续暴露链点 `split.${k}.tip.${i}` 关节（root→tip 链式 parent），位置/旋转从完整 tip 链采样。详见 [usd-bone-export.md](usd-bone-export.md) / bug-fixes.md #2 / bug-fixes.md #9 |
