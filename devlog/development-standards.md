...
- **版本号规范（2026-08 起）**：主版本号**直接对接 git graph**（当前 `0.2.12`），格式 `主版本-Sintaka.dailybuild`（如 `0.2.12-Sintaka.00001`）。`Sintaka` 后面的 dailybuild 为 5 位序号，**每次构建 +1，可累加到 5 位数**（00001..99999），主版本更新时清零。版本号写入 `modules/app-config.js` 的 `APP_VERSION`，显示在 Settings → Version。
