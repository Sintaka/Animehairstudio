...
| 普通发丝单尖端子骨骼（strandTip） | 启用 | 0.2.62：普通非 split 发丝支持单尖端子骨骼（`lock.strandTip`/`lock.strandTipStart`；t-only 权重 + tip 链几何跟随；基础 UI：enable / tip start / tip length / reset + 视口显示；镜像/快照/保存已接；视口拖拽编辑、骨骼 registry、USDA 延后） |
