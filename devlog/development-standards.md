...
- **版本号规范（2026-08 起）**：完整版本号格式 `0.1.5-Sintaka.0.2.<dailybuild>`（当前 `0.1.5-Sintaka.0.2.114`）。`0.1.5` 主版本**与上游保持对齐**（不随本地改动递增）；`Sintaka` 为本地 fork 标记；`0.2` 为分段版本号（功能迭代时更新）；末尾 dailybuild **可直接递增到 5 位数**（如 12 → 13 → … → 12345，分段版本号更新时清零）。版本号写入 `modules/core/app-config.js` 的 `APP_VERSION`，显示在顶栏与 Settings → Version。
