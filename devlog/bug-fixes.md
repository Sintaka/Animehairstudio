...
3. **刘海（split 发丝）线框显示三角面**
   ...
   - **0.2.55 已修复**：面板（三片大刘海 Front Bangs 1/2/3）Split 开口处的退化/反射折叠 quad（Front Bangs 1 有 16 个 179-180° 反射折叠、Front Bangs 3 有 12 个全角重合退化 + 40 个 ≤10.7° 正常曲率）在着色时沿对角线出折痕呈三角观感；`addQuad` 现在跳过退化（角点重合）与反射折叠（两三角法线点积 < -0.999）的 quad，最大二面角降到 ≤10.7°，三角观感消失。原版项目同样存在此问题。
