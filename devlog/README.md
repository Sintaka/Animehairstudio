...
- **新笔刷（已实现）**
  ...
  2. **Scale 笔刷（Scale / Cut·Extend 两种模式）**
     ...
     （其它调整：Smooth 笔刷增加绕切线旋转（twist）平滑，不再只处理位置 P；Scale 默认强度 0.5；导航默认模式改为 Default，左下角新增独立可关闭的小提示框（× 关闭并记忆；语言简写 en/ja/zh-cn）；默认语言英文，日文下笔刷名翻译、中英保持英文；Preview 菜单的 Turntable 点击不关闭菜单可连续 toggle。）
...
- [x] Smooth 增加 twist 平滑；Scale 默认强度 0.5；导航默认 Default + 左下角设置提示（中英日）；Turntable 菜单不关闭
- [x] 设置提示改为独立可关闭小框（en/ja/zh-cn 简写）；ScaleMode 行仅 Scale Brush 显示
