...
| **进行中计划 / In-progress plans** | [in-progress/split-bone-refactor-plan.md](in-progress/split-bone-refactor-plan.md)（Split Spacing/Trim → split 子骨骼）+ [in-progress/unified-bone-model.md](in-progress/unified-bone-model.md)（KineFX 式统一骨骼模型）+ [in-progress/child-sweep-unification.md](in-progress/child-sweep-unification.md)（子发片=默认扫掠+桥接+根部移动）+ [in-progress/bone-system-roadmap.md](in-progress/bone-system-roadmap.md)（骨骼推进路线 + 持久化多骨骼设计）+ [in-progress/panel-split-tip-bones.md](in-progress/panel-split-tip-bones.md)（段尖端子骨骼分析：长短/走向控制）；完成后归档 [plans/archive/](plans/archive/) |
