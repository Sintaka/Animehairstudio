...
- Phase 2.17：子发片封面侧面 4 边面填充（分支 v0.1.4-Side-Topology，在 0.2.42 重写，无 smooth）——间接桥接（至少一条侧边留空）时从直接桥接向洞顶/底 1:1 填 quad 条带，利用 0.3 预留段无三角；版本 0.1.4-Sintaka.0.2.44（0.2.44 修复多行填充的 ringBase 索引偏移——洞侧中间顶点预推到环索引固定前，顶部桥接不再被扰乱）。
