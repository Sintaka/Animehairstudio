...
| **进行中计划 / In-progress plans** | [in-progress/split-bone-refactor-plan.md](in-progress/split-bone-refactor-plan.md)（Split Spacing/Trim → split 子骨骼）+ [in-progress/unified-bone-model.md](in-progress/unified-bone-model.md)（KineFX 式统一骨骼模型）+ [in-progress/child-sweep-unification.md](in-progress/child-sweep-unification.md)（子发片=默认扫掠+桥接+根部移动）+ [in-progress/bone-system-roadmap.md](in-progress/bone-system-roadmap.md)（骨骼推进路线 + 持久化多骨骼设计）；完成后归档 [plans/archive/](plans/archive/) |
...
- 统一骨骼模型 + 子发片扫掠进化 + devlog 整理（0.2.59，分支 0.2.58-panel-split-refactor，**已实施**）：devlog 新开 `in-progress/` 计划文件夹（split-bone-refactor-plan / unified-bone-model / child-sweep-unification），完成后归档到 `plans/archive/`；Panel Split Spacing/Trim 从绝对位移重铸为 **split 子骨骼**（`lock.splitBones`：P + orient 四元数 + spread + 每段 Width/Depth 曲线，混合持久化），`createPanelStrandGeometry` 段内局部 u' + 每段曲线 + **相对缩放**（恒 uStart≤uEnd 根除 crossover），删除 trim/gap 位移；新增 `bonesFor(lock)` 统一骨骼视图（main/split/child 命名空间，主骨骼有子骨骼才架空）+ 段选择器/spread 滑杆/每段曲线编辑（复用 taperCurveEditor segment 模式）/视口段手柄；子发片进化为「默认扫掠 + 桥接 + 根部移动优化」（共享扫掠内核 `strand-sweep.js` `sweepSide`，几何与 0.2.57 字节一致）。验证：Sussurro_v1_0041/0042/0044 三档 11/11 smoke。调研详见 annotations-panel-zipper.md / annotations-split.md（0.2.58 条目），实施见 annotations-split.md / annotations-bridge.md（0.2.59 条目）。
