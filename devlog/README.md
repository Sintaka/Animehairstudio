...
- **新笔刷（已实现）**
  ...
  2. **Scale 笔刷（Scale / Cut·Extend 两种模式）**
     ...
     （其它调整：Smooth 笔刷增加绕切线旋转（twist）平滑，不再只处理位置 P；Scale 默认强度 0.5；导航默认模式改为 Default，左下角浮动框增加小字提示「可在 Settings 修改导航方式与显示语言，支持中英日」；默认语言英文，日文下笔刷名翻译、中英保持英文；Preview 菜单的 Turntable 点击不关闭菜单可连续 toggle。）
