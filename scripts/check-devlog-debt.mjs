...
// 阈值。改这里就改了判据，不要散落在别处。
//
// 这些数字是**量出来的，不是拍的**。第一版取 20 份 / 400KB / 50KB，而实测 in-progress 的
// 不可压缩地板是 **19 份 / 675KB** —— 那 19 份被 `modules/*.js` 与 `app.js` 的代码注释直接
// 引用（"完整 deps 清单在哪查"的活指针），只要引用还在就不能归档。也就是说第一版的线**永远
// 满足不了**，每轮必报。永远报警的线等于装饰：它会被忽略，与"不咬的判据"一样坏
// （同一个缺陷本轮已在 `round-check.mjs` 的 exports 上犯过一次）。
// 我为此把线拍错了**两次**，留档以免第三次：
//   第一版 20 份／400KB —— 低于「被代码钉住」的 19 份/675KB；
//   第二版 25 份／800KB —— 我以为地板就是那 19 份，实测**当前 29 份里有 28 份搬不走**
//     （12 份既被钉住又有未收口项、7 份仅被钉住、9 份仅有未收口项），真实稳态是 839.6KB。
// 两次都是同一个错：拿"理想状态"当地板，而不是拿"今天实际搬不走的量"当地板。
// 第三次调整（0.2.145）：地板真的降了，因为钉住 15 份 refactor-map 的 32 处代码注释指针
// 被清掉了（27 处纯冗余、5 处已内联），15 份随之从 in-progress/ 归档进 archive/。
// 实测新地板：**15 份 / 416KB**，单份最大 74.86KB（panel-split-tip-bones.md，四舍五入 75KB）。
// 按同一套「~15-20% 余量」拍线（跟第二版 950/839.6、100/84.2 的余量比例一致）：
// 文件数 15→20、总量 416→480KB、单份 75→90KB。
// 自检口径：改完必须确认三条**当前都不触发** —— 否则它又在报稳态。
// 地板会随代码注释被清掉而下降，届时把线一起下调；线要跟着地板走，不要反过来。
const MAX_FILES = 20;
const MAX_TOTAL_KB = 480;
const MAX_SINGLE_KB = 90;
...
// 第二轮补词（同样来自实测，不是补想象）：`后续再议`/`暂不`/`暂缓`/`另议`/`留待`。
// 缺口是子智能体**读**出来的而我的正则漏了 —— `child-sweep-unification.md` 写着
// 「strand 单 split 不并入本次内核（独立几何，后续再议）」，我判它可归档、它判不可，它对。
// ⇒ 人读文档仍然强于词表匹配；这个判据只能用来**缩小**人要读的范围，不能替代判断。
const OPEN_RE = /未做|待办|待定|待裁决|待用户|待验证|待确认|尚未|仍未解|先不做|先不管|先留着|路线图|Phase\s*2|TODO|-\s*\[\s*\]|后续再议|暂不|暂缓|另议|留待/i;
