...
const findings = { oversize: [], archivable: [], dense: [] };
let scanned = 0;
...
const hit = findings.oversize.length + findings.archivable.length + findings.dense.length;

if (hit === 0) {
  if (verbose) console.log(`[PASS] devlog-debt: 扫描 ${scanned} 份 .md，三条线均未触及。`);
  process.exit(0);
}
