...
- **Split Spacing fix (0.2.125)** — on ordinary strands the Split Spacing slider went completely dead after any `+`/`−`, Tip Length, Reset Split Tips, or tip drag (it moved the readout but not the mesh). It is now a **global brush**: dragging it writes into every tube, which does overwrite per-tube values set with Segment Spread — that is what "for every split tube" always meant.
